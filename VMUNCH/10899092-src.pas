program vmunch;
const x: array [1..3] of integer=(-1,0,0);
      y: array [1..3] of integer=(0,1,-1);
...
procedure try(f, t, k: longint);
 ...
 begin
  for i:=1 to 4 do
   if (mapb[f+x[i], t+y[i]]=false) and (map[f+x[i], t+y[i]]<>'*') and (k+1<min) then begin
